...
- **public/index.php**: Single-list endpoint (fast lista 247). **Directus-first** sedan 2026-09: Koha-RSS:en ger bara listmedlemskap (1 request), metadata i bulk från Directus via `processRssFeedFromDirectus()`. Fail-throttle (`fail_index.flag`) + snapshot (`snapshot_index.json`) som shelf.php.
- **public/list.php**: Dynamic multi-list endpoint (`?id=XXX`). **Directus-first** sedan 2026-09 — samma mönster som shelf.php: 1 Koha-request för RSS:en, metadata i bulk från Directus, fail-throttle (`fail_list_{id}.flag`) + snapshot (`snapshot_list_{id}.json`). `?id` valideras med `ctype_digit` (positivt heltal); tidigare `is_numeric()` släppte igenom `1e5`/`0.5` som gav egna cache-filer för samma lista.
- **public/latest.php**: Latest books endpoint - **Directus-first HELT utan Koha-beroende**. Upptäckt av nyaste biblios från `kft_koha_biblios` (ofiltrerat) eller `kft_koha_items` (filtrerat), metadata i bulk via `getBiblioMetadataFromDirectus()`, omslag från lokal bildcache + Syndetics (`resolveBookImageLocal()`). Directus synkas nattligen kl 03:00, så datat kan släpa upp till ett dygn. Stöder `?item_type_id=TYPE1,TYPE2`, `?location=MAG,SVSPRAK` (Kohas placering) och `?ccode=GYMN,VUX` (collection_code/avdelning). Filter kombineras: OR inom fält, AND mellan fält. Värden är skiftlägesokänsliga (normaliseras till uppercase) och valideras strikt av `parseFilterParam()` — bara `[A-Z0-9_-]`, max 32 tecken och 20 värden per fält, annars HTTP 400. `limit` klämms till 1–50; golvet är kritiskt eftersom `limit=-1` i Directus betyder *obegränsat* och en enda request annars skulle dra hem hela katalogen. Vid Directus-fel serveras last-known-good-snapshot med `stale: true`, och en fail-flagga per limit+filterkombination (delad mellan json/xml) throttlar nya försök till 1 per 30 min (`KOHA_FAIL_THROTTLE`), samma mönster som shelf.php. Tomt resultat cachas aldrig; befintlig snapshot föredras framför plötsligt tomt svar.
- **public/shelf.php**: Shelf-list endpoint (`?shelfnumber=XXX&format=json|xml`), används av JS-widgeten `koha-shelf.js`. `shelfnumber` valideras med `ctype_digit` (positivt heltal, utelämnat ger 247); tidigare `intval()` släppte igenom negativa tal som gav egna cache-filer och Koha-anrop. **Directus-first**: Koha-RSS:en ger bara hyllmedlemskap (1 request), bokmetadata hämtas i bulk från Directus (`getBiblioMetadataFromDirectus()`) och omslag från lokal bildcache — inga per-bok-anrop mot Koha (det var vad som triggade ImCodes fail2ban aug 2026). Vid Koha-fel serveras last-known-good-snapshot med HTTP 200 + `stale: true`; en fail-flagga throttlar Koha-försök till 1 per 30 min per hylla (`KOHA_FAIL_THROTTLE` i `.env`).
- **public/book.php**: Single-book endpoint (`?biblionumber=XXX&format=json|xml`). `biblionumber` valideras med `ctype_digit` (positivt heltal, annars 400 via `sendErrorResponse()`). Enda kvarvarande endpoint som går direkt mot Koha-API:t — Directus saknar fem av fälten (`creation_date`, `koha_timestamp`, `copyright_date`, `lc_control_number`, `serial`), så en övergång vore en brytande API-ändring. Amplifieringen dämpas i stället av **negativ cache**: en miss skriver `miss_book{id}.flag` (TTL `CACHE_TTL_BOOK_MISS`, standard 1h) så att iteration över obefintliga biblionummer inte ger ett Koha-anrop per request. Delad fail-flagga `fail_book.flag` throttlar vid Koha-fel.
- **public/item-types.php**: Item types listing endpoint - fetches all available item types from Koha API with no caching
- **public/js/**: JS-widget-distribution (`koha-shelf.js` + demo). Widgeten bygger kort med template-strängar och `innerHTML`, så ALLT som kommer från API-svaret måste genom `escapeHtml()` — inklusive värden som hamnar i attribut (`src`, `alt`, `href`). `escapeHtml()` escapar `& < > " '`; escapa aldrig med `textContent`→`innerHTML`-tricket, det lämnar citattecken orörda och duger bara i textkontext. Trunkera före escaping, annars kan en HTML-entitet kapas mitt itu.
...
- `directusItemsGet($collection, $queryParams)` - Intern hjälpare för GET mot Directus items-API, returnerar `['success', 'error', 'rows']`
- `getLatestBiblioIdsFromDirectus($limit)` - Senaste aktiva biblio_ids från `kft_koha_biblios` sorterat på biblio_id fallande (ersätter Kohas `/biblios?_order_by=-biblio_id` för latest.php)
- `getFilteredBiblioIdsFromDirectus($itemTypes, $locations, $ccodes, $limit)` - Distinkta biblio_ids (senaste först) från `kft_koha_items` filtrerat på item_type_id/location/collection_code (AND mellan fält, OR inom). Sorteringen `-biblio_id` lägger exemplar av samma biblio adjacenta, så dedupliceringen streamar och avbryter vid limit. Returnerar även `item_types_map` per biblio.
...
- **All query-input valideras vid ingången**, inte "risken är låg för att datakällorna är kontrollerade": `?id`/`?biblionumber`/`?shelfnumber` med `ctype_digit` + `intval` (positivt heltal, annars 400), filter med `parseFilterParam()`, `?limit` med golv OCH tak. Regeln är att varje värde som når ett filnamn, en URL eller ett Directus-filter måste vara teckenvaliderat först — katalogdata räknas också som otrodd indata (se `getFirstIsbn()`).
- **Kontrollera `is_string()` innan `trim()`/`strtolower()` på `$_GET`-värden.** PHP gör `?param[]=x` till en array, och strängfunktioner på en array kastar `TypeError` under PHP 8 — ett 500-svar i stället för 400. `intval()` på en icke-tom array ger dessutom `1`, så `?biblionumber[]=x` slog tidigare upp bok 1. Alla endpoints och `parseFilterParam()` avvisar nu arrayer explicit.
- **`X-Content-Type-Options: nosniff` sätts i alla endpoints.** Svaren är JSON/XML med korrekt Content-Type; headern gör att webbläsaren aldrig kan tolka dem som HTML.
- **Semgrep-fynden `tainted-filename` och `echoed-request` är falska positiver.** Regelns sanerarlista innehåller bara `basename()`/`realpath()` respektive `htmlspecialchars()`, så `intval()`, `ctype_digit()` och `in_array()`-whitelist räknas inte, och smutsen följer med genom stränginterpolering till `$cacheFile` och `echo`. Kvittera dem med den motiveringen. Kör ALDRIG `htmlentities()` på utdata — det förstör JSON-svaren. Se även README-avsnittet "Statisk kodanalys".
...
- **`branches` vs `available_branches`**: `branches` = hemfilial (`home_library_id`) för alla aktiva exemplar. `available_branches` = hemfilial för exemplar som är tillgängliga **och hemma**: ej utlånade/referens/skadade/förkomna/kasserade *och* `holding_library_id` = `home_library_id` (saknad holdingbranch räknas som hemma). Ett Falkenberg-exemplar som tillfälligt står i Slöinge räknas alltså inte som inne någonstans – Slöinge-filtret ska bara ge titlar som *hör hemma* där. Samma villkor styr `available_items`. Driver "Finns inne på"-filtret i sökmodulen. Beräknas i `aggregateItemsByBiblio()` i `prepare_embedding_text.php` och ingår **inte** i embedding-texten (skulle tvinga omembedding); `available_items` ingår däremot i texten ("Exemplar: N totalt, M tillgängliga"), så en ändrad tillgänglighetsregel omembeddar de titlar vars räknevärde ändras.

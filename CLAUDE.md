...
- **public/book.php**: Single-book endpoint (`?biblionumber=XXX&format=json|xml`). Enda kvarvarande endpoint som går direkt mot Koha-API:t — Directus saknar fem av fälten (`creation_date`, `koha_timestamp`, `copyright_date`, `lc_control_number`, `serial`), så en övergång vore en brytande API-ändring. Amplifieringen dämpas i stället av **negativ cache**: en miss skriver `miss_book{id}.flag` (TTL `CACHE_TTL_BOOK_MISS`, standard 1h) så att iteration över obefintliga biblionummer inte ger ett Koha-anrop per request. Delad fail-flagga `fail_book.flag` throttlar vid Koha-fel.
